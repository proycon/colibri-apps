...
def processrelations(type, func, pattern, nodes, edges, classdecoder, colors, relationtypes="",secondorderedges=False):
...
class Root:
    ...
    def query(self, pattern, relationtypes=""):
        ...
        if pattern in self.patternmodel:
            ...
            processrelations('c',self.patternmodel.getsubchildren, pattern, nodes, edges, self.classdecoder, colors, relationtypes,True)
            processrelations('p',self.patternmodel.getsubparents, pattern, nodes, edges, self.classdecoder,  colors,relationtypes,True)
            ...
            processrelations('t',self.patternmodel.gettemplates, pattern, nodes, edges,self.classdecoder,  colors,relationtypes,True)
        ...
        html += "<body><div id=\"graph\" style=\"width: 90%; height:90%; \"></div>"
        if not jscode:
            html += "<p>Pattern not found</p>"
        html += "<form action=\"/query/\" method=\"post\"><input name=\"pattern\" /><input type=\"submit\"></form></body></html>";
        return html
